...
# ╔═╡ 88d8fb78-b913-11ed-0076-ef05a0548ce8
using Molly

# ╔═╡ ebd4e6d1-082e-45e4-bd45-fa6c3d9aa39d
using DelimitedFiles

# ╔═╡ 4a7f9e65-a4d1-4ec7-8137-a48a0bdbe9ce
import Plots

# ╔═╡ e28b37ea-9084-4d78-af9f-8efd62a59fb4
begin
	n_atoms = 200
	mass   = 15.0u"u"
...
# ╔═╡ 2ef6612d-9e4c-45ff-81c2-45c867fec963
begin
	boundary = CubicBoundary(4.0u"nm", 4.0u"nm", 4.0u"nm")
	coords   = place_atoms(n_atoms, boundary; min_dist=0.3u"nm")
end

# ╔═╡ c0c6aaff-3032-45dd-9619-8d161f625c72
begin 
	temp       = 50.0u"K"
	velocities = [velocity(mass,temp) for i in 1:n_atoms] # sample from M-B dist
end

# ╔═╡ 405efad6-893f-48b1-bd03-6e5b16c29775
pairwise_inters = (LennardJones(),)

# ╔═╡ 63f89ac7-671e-4629-9478-e25df8196341
sys = System(
	atoms     		= atoms,
...
# ╔═╡ 1859afea-acda-461f-add7-53c9c1655527
simulator = VelocityVerlet(
			dt=0.002u"ps",
			coupling=AndersenThermostat(temp, 1.0u"ps")
)

# ╔═╡ fe3984f4-0b7d-4423-bcbb-f51c397dc328
simulate!(sys,simulator,5000)

# ╔═╡ 3b9633b4-7656-4ffb-b209-42d73672ba93
md"I would expect the following cell to auto-update every time I run simulate, but it does not"

# ╔═╡ c6006e3b-ca2f-4f63-92dd-4c87f51a35f0
length(sys.loggers.coords.history)

# ╔═╡ db747f31-e555-4d7d-8bc4-ccbb2501cd30
md"Whatever"

# ╔═╡ e4411b70-232a-4532-b539-857f6ab2decb
begin
...
# ╔═╡ e1e34d1b-97ed-4366-94e4-632c86ed8e04
Plots.plot3d(atom1x,
	   		atom1y,
			atom1z,
			marker=1,
...
# ╔═╡ 1b630094-349c-400d-80b0-9b4d066baefb
typeof(atom1x)

# ╔═╡ 3d105636-a839-43c0-a829-74379c36b67e
begin
	open("atom1_coords","w") do io
		writedlm(io,[atom1x atom1y atom1z]) # don't put commas!
	end
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "5b2ea4e97a201f841e340de652fdc597cf09bddc"
...
git-tree-sha1 = "68f4276f3f43ba0086d14e761b6dd7517674956a"
...
[[deps.Molly]]
deps = ["AtomsBase", "BioStructures", "CUDA", "CellListMap", "ChainRulesCore", "Chemfiles", "Colors", "Combinatorics", "DataStructures", "Distances", "Distributions", "EzXML", "FLoops", "ForwardDiff", "KernelDensity", "LinearAlgebra", "NearestNeighbors", "Random", "Reexport", "Requires", "SparseArrays", "StaticArrays", "Statistics", "Unitful", "UnitfulChainRules", "Zygote"]
git-tree-sha1 = "682095d6f15017ae0fd4986caff9c5c444b21bc2"
