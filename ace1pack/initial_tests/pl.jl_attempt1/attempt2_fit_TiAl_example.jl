...
function mock_export2lammps(fname, lb::LinearBasisPotential)
    ...
    export2lammps(fname, mock_ace1model, rpib) # I can't seem to call the export2lammps that directly uses the potential, not ace1model
...
mock_ace1model = ACE1x.ACE1Model(super_basis,params,vref,pot,Dict())
#ACE1pack.export2lammps(fname, mock_ace1model) 

```
export2lammps(fname, mock_ace1model,rpib)
```
fname = "newtest.yace"
IP = mock_ace1model.potential
